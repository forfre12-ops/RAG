...
def split_for_training(
    ...
    *,
    ...
    by_grade: dict[str, list[tuple[str, dict]]] = defaultdict(list)
    for r in records:
        ...
        if h in seen:
            ...
            continue
        seen.add(h)
        by_grade[str(grade_of(r))].append((h, r))

    silver: list[dict] = []
    holdout: list[dict] = []
    per_grade: dict[str, dict] = {}
    for grade, items in by_grade.items():
        items.sort(key=lambda hr: hr[0])  # text_hash 정렬 = 결정적·균일
        n = len(items)
        hn = _holdout_count(n, holdout_frac)
        for i, (_, r) in enumerate(items):
            if i < hn:
                holdout.append({**r, "split_role": HOLDOUT, "eval_note": HOLDOUT_NOTE})
            else:
                silver.append({**r, "split_role": SILVER})
        per_grade[grade] = {"n": n, "holdout": hn, "silver": n - hn}
...
def split_run_dir(
    ...
    *,
    ...
    result = split_for_training(
        records, existing_texts=existing, holdout_frac=holdout_frac, text_key=text_key,
    )
