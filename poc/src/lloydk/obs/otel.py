...
def span(name: str, **attrs):
    ...
    try:
        tracer = trace.get_tracer("lloydk")
    except Exception:  # noqa: BLE001
        yield None
        return
...
# 환경변수 LLOYDK_LOG_JSON이 truthy일 때만 root logger에 JSON 포매터를 적용한다.
...
def setup_logging() -> bool:
    """opt-in JSON 구조화 로깅 설정. LLOYDK_LOG_JSON truthy일 때만 적용.

    Returns:
        True if JSON logging applied, False if no-op (기본).
    """
    if not _truthy(os.getenv("LLOYDK_LOG_JSON")):
    ...
    logger.info("JSON structured logging enabled (LLOYDK_LOG_JSON)")
    return True
