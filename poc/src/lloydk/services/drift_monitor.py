...
#   LLOYDK_DRIFT_STATE_PATH가 있으면 그것을, 없으면 centroid 파일과 같은 디렉터리에
#   drift_state.json으로 둔다(centroid 경로를 tmp로 바꾸는 테스트가 자연히 격리됨).
def _default_state_path() -> str:
    explicit = _os.environ.get("LLOYDK_DRIFT_STATE_PATH")
    if explicit:
        return explicit
...
#   운영에서 단발 스파이크 억제가 필요하면 env LLOYDK_DRIFT_CONSECUTIVE_REQUIRED=2~3으로
...
_DEFAULT_CONSECUTIVE_REQUIRED = int(
    _os.environ.get("LLOYDK_DRIFT_CONSECUTIVE_REQUIRED", "1")
)
# cosine_mean 이동 임계 기본값 — 0(비활성)이면 KL만으로 판정(동작보존).
_DEFAULT_COSINE_SHIFT_THRESHOLD = float(
    _os.environ.get("LLOYDK_DRIFT_COSINE_SHIFT_THRESHOLD", "0.0")
)
