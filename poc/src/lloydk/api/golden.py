...
$('refresh').onclick=load;$('filter').onclick=load;$('action').onchange=toggleGrade;$('save').onclick=save;$('openUpload').onclick=()=>$('modal').classList.add('show');$('cancelUpload').onclick=()=>$('modal').classList.remove('show');$('upload').onclick=upload;$('ledgerReload').onclick=loadLedger;$('ledgerFilter').onchange=loadLedger;load();
</script></body></html>"""
...
def _render_console_login_html() -> str:
    ...
    return (
        ...
        "<title>SPECLEDGER | GOLDSET OPS 로그인</title><style>"
        ...
        "padding:0 40px;gap:16px}.mark{width:36px;height:36px;background:#111;border-radius:7px;"
        "position:relative}.mark:after{content:'';position:absolute;background:var(--red);"
        "width:17px;height:6px;transform:skew(25deg);left:10px;top:12px}"
        ...
        "</style><body><header class=\"top\"><div class=\"mark\"></div>"
        '<div class="brand">SPECLEDGER</div>'
        '<div class="product">GOLDSET OPS · LOGIN</div></header>'
