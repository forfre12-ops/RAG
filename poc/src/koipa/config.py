...
_PROFILE_DEFAULTS: dict[str, dict[str, object]] = {
    ...
    "onprem-local": {
        ...
        # 발동·메타존재율은 koipa_metadata_floor_applied_total / _classify_metadata_present_total로 가시화.
...
class Settings(BaseSettings):
    ...
    minio_bucket_docs: str = "koipa-docs"
    minio_bucket_models: str = "koipa-models"
    ...
    # [C2 2026-08-17] 이제 화면 기본값이 아니다 — 서명 화면에서 검수자 입력란 자체를 없앴다.
    # 남은 용도는 하나: golden_tiers.is_human_reviewer 가 **이 이름과 같은 서명을 거부**한다
    # (DEF-2026-53). 즉 "화면이 채워 주던 이름" 을 사람 검수자로 인정하지 않기 위한 값이다.
    # ⚠ 비우면 `if default_rid and rid == default_rid` 의 앞 조건이 거짓이 되어 거부가 통째로
    #   사라진다. 값을 지우지 말 것.
    ...
    # env: AUDIT_CHAIN_SECRET(필드명) 또는 KOIPA_AUDIT_CHAIN_SECRET(audit_chain._chain_secret
    ...
    # middleware._fail_closed_enabled가 이 필드 우선, 없으면 env KOIPA_AUDIT_FAIL_CLOSED_HIGH_RISK
    ...
    #   · 고객사 워커 4GiB 에서 같은 태스크를 발화하니 **40초 만에 OOMKilled** 됐다.
    #     (메모리는 CPU 속도와 무관하므로 하드웨어가 빨라져도 그대로 죽는다.)
    #     enable_training=False 여도 enable_incremental_retrain 이 학습 가드를 연다.
    #   · 새벽에 죽으면 화면에 아무 신호가 없다.
    # 원래 목적("교정을 반출하지 않고 현장 반영")은 수동 트리거로도 동일하게 달성된다.
    # True 로 켜면 종전 동작(02:00 KST 무인 발화)이 복원된다 — 워커 메모리를 함께 올릴 것.
    # (enable_incremental_retrain 은 그대로 둔다 — 그 플래그는 '이 노드가 재학습을 할 수 있는가'
    #  이고, 이 설정은 '사람 없이 정해진 시각에 자동으로 할 것인가'로 축이 다르다.)
    enable_nightly_retrain_schedule: bool = False

    # ── 임베딩 드리프트 감지 (기본 OFF) ────────────────────────────────────────────
    # [스코프 결정 2026-08-08] 드리프트 감지는 **요건이 아니다** — RTM 에 해당 행이 없고,
    # 담당 요건(FUN-003·004·005·022·023·024) 어디에도 없다. FUN-004 는 "분류 모델 학습"까지이며
    # 학습 이후의 분포 감시는 범위 밖이다. 그래서 기본값을 OFF 로 둔다.
    #
    # 코드는 남긴다(삭제 아님) — 고객사 현장에서 실제 문서 분포가 바뀌는 것을 잡을 유일한 장치라
    # 필요해지면 이 플래그만 켜면 된다. 다만 기본으로 켜 둘 이유가 없다:
    #   · 비용(실측 2026-08-08): 학습 끝에 train centroid 를 만들려고 학습 표본 200건을 KURE-v1 로
    #     임베딩한다. 69분 재학습 중 **30분 10초**가 이 단계였다(학습 자체는 34분). 고객사 야간
    #     무인 배치에 매번 이만큼이 더 붙는다.
    #   · 노이즈: 운영 표본이 적으면 의미 없는 경보가 뜬다. 실측에서 sample=2 로 alert=True
    #     (kl=4.8614) 가 나왔다 — 표본 2건짜리 판정이라 신호가 아니다.
    ...
    # 억제 발동은 koipa_kill_gate_autoconfirm_suppressed_total로 가시화. 캐시된 tripped 상태만 읽어
    ...
    # 보안 민감 도메인이라 기본 True. dryrun/테스트에서 비활성하려면 KOIPA_PII_MASKING_ENABLED=0.
...
def assert_production_credentials() -> None:
    ...
    KOIPA_SECRETS_BACKEND=env(기본)는 사실상 no-op이라 dev 동작에 영향 없음.
    """
    ...
    if settings.storage_backend in ("minio", "seaweedfs", "s3") and not settings.minio_secret_key:
        missing.append("KOIPA_MINIO_SECRET_KEY")
    # NFR-SEC-01: 감사체인 HMAC 비밀키. 미설정 시 키 없는 sha256 폴백 → 과거 row 재작성 가능.
    # 실효 시크릿(필드 또는 KOIPA_ env) 기준으로 판정 — 둘 다 비면 운영 startup 차단.
    if not (settings.audit_chain_secret or os.environ.get("KOIPA_AUDIT_CHAIN_SECRET", "").strip()):
        missing.append("KOIPA_AUDIT_CHAIN_SECRET")
    if missing:
        raise RuntimeError(
    ...
    from koipa.services.idempotency import (  # noqa: PLC0415
        assert_multiworker_idempotency_safe,
    )
    assert_multiworker_idempotency_safe()

    # [정합성] 동일 사유 — 멀티워커에서 async job 저장소가 in-memory 폴백이면 워커 간 job 유실.
    from koipa.services.job_store import (  # noqa: PLC0415
...
def fill_from_secrets_manager() -> dict:
    ...
    try:
        from koipa.services.secrets_manager import get_secrets_manager  # noqa: PLC0415
