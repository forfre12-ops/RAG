...
여기 있는 규칙은 **양쪽이 함께 쓴다.** 한쪽에서만 고치면 다시 어긋나므로,
화면 고유 부품(후보 카드·모달·필터 등)만 각자 파일에 두고 골격은 여기서 가져간다.

⚠ 값의 출처는 manage.html 이다 — 그쪽이 감리 정본(화면설계서 UI-01/UI-04)에 실린 화면이다.
  여기서 값을 바꾸면 두 화면이 같이 바뀐다.
"""
