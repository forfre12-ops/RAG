...
KPIS: list[KPI] = [
    ...
    # S10 분류 근거 일치성 (W11 확장 · 2026-09-05 개명)
    #
    # 종전 이름은 'RAG 인용 충실도'였다. RAG 는 미채택으로 실행 경로에서 걷혔고,
    # 이 시나리오가 실제로 재는 것은 **분류 결과의 evidence 가 입력 본문에 있는가**다 —
    # 검색과 무관하다. 이름이 사실과 다르면 보고서를 읽는 사람이 없는 기능을 재고
    # 있다고 읽는다.
    KPI("S10.1", "S10", "grounded_ratio", "ratio", "ge", 0.70, "mean"),
    KPI("S10.2", "S10", "evidence_count", "count", "gt", 0, "min"),
    # 평가셋이 등급별 시드 키워드 1개로 합성돼 있어, 모델이 다른 등급으로 판정하면 그 등급의
    # 키워드는 본문에 없다 → 인용이 멀쩡해도 불일치로 집계된다. 223 실측(2026-08-17) 0.60 의
    # 내역은 **판정 차이 15건 · 근거 결함 0건** 이었다. 이 값을 "근거가 40% 망가졌다" 로 읽으면
    # 틀린다 — 시나리오가 두 원인을 갈라 로그에 남긴다(근거 결함만이 품질 문제다).
