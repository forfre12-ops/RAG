...
def human_reviewer_rejection_reason(reviewer_id: object) -> str:
    ...
    # [2026-08-20 사용자 결정] 프리필 신원 거부를 뺀다 — 로그인은 항상 되어야 한다.
    # 8/17 에 넣었던 거부 규칙 때문에 223 에서 검수가 한 건도 진행되지 못했다
    # (preflight reviewer_rejected · 서명 0건).
    # ⚠ 잃는 것: 원장에 사람별 구분이 남지 않는다. reviewer_id 는 "이 조직이 검수했다" 까지만
    #   말해 준다. 사람별 책임 추적이 필요하면 프리필을 끄고 사람마다 토큰을 발급한다
    #   (scripts/setup_console_test_login.py --sub).
    # ⚠ 그래서 프리필 이름은 실계정이어야 한다. 아래 기계·시연 접두사 검사는 그대로 두어
    #   kl-admin-test 같은 이름은 계속 거부한다.
