...
def human_reviewer_rejection_reason(reviewer_id: object) -> str:
    ...
    # [2026-08-20 사용자 지시] 프리필 신원 거부를 **뺀다.**
    #
    # 2026-08-17 에 넣었던 규칙이다 — 로그인 화면이 모두에게 나눠 주는 이름으로는 서명을
    # 막았다. 그 결과 223 에서 검수가 한 건도 진행되지 못했다(preflight `reviewer_rejected`,
    # 서명 0건). 대안은 사람마다 토큰을 발급해 **브라우저마다 붙여넣게** 하는 것인데,
    # 시연 중에 그것이 막히면 검수 자체를 못 보여 준다.
    #
    # 사용자 결정: **로그인은 항상 되어야 한다.** 프리필 신원 그대로 서명할 수 있게 한다.
    #
    # ⚠ 잃는 것을 분명히 적는다 — 원장에 **사람별 구분이 남지 않는다.** 그 주소를 여는
    #   누구나 같은 이름으로 서명하므로 `reviewer_id` 는 "이 조직이 검수했다" 까지만
    #   말해 준다. 사람별 책임 추적이 필요해지면 프리필을 끄고 사람마다 토큰을 발급하는
    #   방식으로 되돌려야 한다(`scripts/setup_console_test_login.py --sub`).
    # ⚠ 그래서 프리필 이름은 **실계정 이름**이어야 한다. `kl-admin-test` 같은 시연용
    #   이름이 원장에 남으면 검수 기록으로 쓸 수 없다. 아래 기계·시연 접두사 검사는
    #   그대로 두어 그런 이름은 계속 거부한다.
