...
from koipa.adapters.reranker import Reranker, get_reranker
from koipa.adapters.vectorstore.base import SearchHit, VectorStore
from koipa.rag.query_expansion import expand_hybrid, expand_llm, expand_rule
...
def _record_hybrid_degrade() -> None:
    ...
    retrieval은 도메인 독립 코어이므로 koipa.api에 강결합하지 않는다. 메트릭은
...
def _rerank_hits(
    *,
    ...
    from koipa.obs.otel import span  # noqa: PLC0415
    try:
        with span(
            "ml.reranker.rank",
