...
def _build_minio() -> ObjectStorage:
    from koipa.adapters.storage.minio_store import MinioStorage  # noqa: PLC0415
...
def _build_seaweedfs() -> ObjectStorage:
    from koipa.adapters.storage.seaweedfs_store import SeaweedFSStore  # noqa: PLC0415
    from koipa.config import settings  # noqa: PLC0415
...
def _maybe_encrypt(storage: ObjectStorage) -> ObjectStorage:
    ...
    if not getattr(settings, "storage_encryption_enabled", False):
        return storage
    from koipa.adapters.storage.encrypted_store import EncryptingStorage  # noqa: PLC0415
