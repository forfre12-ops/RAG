...
def list_review_queue(
    ...
    그러므로 같은 한 시간을 쓸 때 잡히는 미탐이 많은 순서는 이렇다.
        ...
        ① 예측 등급이 낮은 것 먼저 (level_order 내림차순 — S3 → S2 → S1 → TS)
           S3 로 본 것이 실은 TS 면 치명적 미탐이다. 반대로 **예측 TS 는 정의상 미탐일
           수 없다** — 위에 등급이 없어 내려갈 수만 있다. 뒤로 미루는 것이 구조적으로 옳다.
        ② 같은 등급이면 신뢰도가 낮은 것 먼저 (경계에 가까운 것)
        ③ 그래도 같으면 오래된 것 먼저 — 기아 방지. risk 정렬에서도 FIFO 가 마지막
           기준으로 남아 있어야 낡은 항목이 영원히 안 뜨는 일이 없다.

    ⚠ **효과의 크기를 부풀리지 말 것**(실측 2026-09-10,
      `scripts/measure_review_order_yield.py` · v3_final800 검수 대기 800건 · 미탐 161건).

          정렬        k=20   k=50   k=100   k=200
          fifo           5     10      18      38
          risk           5     11      28      45
          oracle        20     50     100     161   (도달 불가 상한)

      k=100 에서 18→28 로 늘지만 **k=20 에서는 차이가 없다.** 이유는 미탐이 등급으로
      갈리지 않기 때문이다 — 예측 S1 22.4% · S2 22.3% 로 사실상 같고, 신뢰도로도
      안 갈린다(0.0~0.5 19.7% · 0.5~0.7 21.6%; AUROC 0.58 과 같은 이야기).
      실제로 얻는 것은 대부분 **미탐일 수 없는 구간(예측 TS)을 뒤로 미루는 것**이다.

      그러므로 정렬은 레버가 아니다. 레버는 큐에 무엇이 들어오는가(게이트)와 모델이다.
