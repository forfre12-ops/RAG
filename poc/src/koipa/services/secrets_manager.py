...
# env KOIPA_SECRETS_CACHE_TTL_SEC로 조정(기본 300초). 0 이하면 캐시 비활성(매번 조회).
def _default_cache_ttl() -> float:
    raw = os.getenv("KOIPA_SECRETS_CACHE_TTL_SEC", "300").strip()
    try:
        return float(raw)
    except ValueError:
        return 300.0
...
def get_secrets_manager() -> SecretsManager:
    ...
    backend = os.getenv("KOIPA_SECRETS_BACKEND", "env").lower()
    if backend == "vault":
        _default = VaultSecretsManager(
            url=os.getenv("VAULT_ADDR", "http://localhost:8200"),
            token=os.getenv("VAULT_TOKEN", ""),
            mount=os.getenv("VAULT_MOUNT", "secret"),
            path=os.getenv("VAULT_PATH", "koipa"),
        )
    elif backend == "aws":
        _default = AwsSecretsManager(
            secret_id=os.getenv("AWS_SECRET_ID", "koipa/prod"),
