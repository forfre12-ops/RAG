...
function hdr(json=true){const h={};if(json)h['Content-Type']='application/json';return h}function msg(v,bad=false){const e=$('result')||document.createElement('div');e.textContent=v;e.className=bad?'result error':'result';if(!e.parentNode){e.id='result';document.querySelector('.toolbar').after(e)}}function qurl(){let q=new URLSearchParams();['status','grade','origin','query','review_batch'].forEach(id=>{let v=$(id).value.trim();if(v)q.set(id,v)});return api+(q.size?'?'+q:'')}async function req(url,opt={}){if(window.__GOLDEN_PREVIEW__){const p=window.__GOLDEN_PREVIEW__;if(opt.method)throw new Error('로컬 미리보기는 읽기 전용입니다. 업로드·등급 변경·폐기는 KL 콘솔 배포 후 사용할 수 있습니다.');if(url.endsWith('/session'))return {actor_id:'로컬 미리보기 · 읽기 전용'};const id=url.split('?')[0].slice((api+'/').length);if(id&&p.by_id[id])return p.by_id[id];return p.list}let r=await fetch(url,{...opt,credentials:'same-origin',headers:{...hdr(opt.json!==false),...(opt.headers||{})}});if(!r.ok)throw new Error((await r.text())||r.status);return r.json()}
