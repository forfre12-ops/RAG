...
def proxy_gold_candidate_decision(
    ...
    # [2026-09-11] 검수자도 결정을 기록한다 — 등급 결정이 검수자의 일이다. 평가정답 승격(promote)은 관리자만.
    auth: dict = Depends(require_role("admin", "kl_backend", "reviewer")),
) -> ProxyGoldCandidateDecisionResponse:
...
if(!src&&!basis)throw new Error('원천 위치나 사용 권한 근거 중 하나는 적어야 합니다.');
...
$('refresh').onclick=load;$('promote').onclick=async()=>{const b=$('promote');b.disabled=true;note('promoteMsg','승격하는 중입니다…');try{const r=await req(api+'/promote',{method:'POST',body:JSON.stringify({publish:true})});let t='승격 '+r.locked+'건 · 누적 평가정답 '+r.locked_total+'건(실문서 '+r.real_locked+'건)';if(r.rejected)t+=' · 서명으로 인정되지 않은 결정 '+r.rejected+'건';if(!r.published)t+=' · 배포 판정에는 아직 반영되지 않았습니다(서버 설정 확인 필요)';note('promoteMsg',t);await load();}catch(e){note('promoteMsg','승격하지 못했습니다. '+(e.message||''),true)}finally{b.disabled=false}};$('filter').onclick=load;$('action').onchange=toggleGrade;$('provSave').onclick=saveProv;$('save').onclick=save;$('openUpload').onclick=()=>$('modal').classList.add('show');// [2026-08-18] 드롭존을 실제로 배선한다. 종전에는 class="drop" 로 **생긴 것만**
...
def _login_prefill_block() -> str:
    ...
    return (
        ...
        "이 주소를 여는 누구나 그 토큰의 권한으로(관리자 토큰이면 관리자로) 들어올 수 있으므로 외부 공개 환경에서는 "
...
# 검수자(reviewer)로 연 후보 관리 화면에서 감추는 관리자 전용 조작 — 업로드 · 출처 기록 · 평가정답 승격.
# 서버가 이미 403 으로 막는 조작이다(권한은 그대로). 늘 실패할 버튼을 보이지 않게 할 뿐이다.
_REVIEWER_VIEW_CSS = "<style>#openUpload,#promote,#provBox{display:none!important}</style>"
_REVIEWER_VIEW_NOTE = (
    '<div class="note" style="margin:10px 34px">검수자 권한으로 열었습니다 — 등급 결정과 서명을 할 수 '
    "있습니다. 문서 업로드 · 출처 기록 · 평가정답 승격은 관리자가 합니다.</div>"
)


def _is_console_admin(auth: dict) -> bool:
    roles = set(auth.get("actor_roles") or ())
    if not roles and auth.get("actor_role"):
        roles = {auth["actor_role"]}
    return not roles.isdisjoint(_CONSOLE_ADMIN_ROLES)
...
def proxy_gold_candidate_manager_html(
    ...
    [2026-09-11] 검수자(reviewer) 역할도 연다. 종전에는 admin·kl_backend 만 받아 검수자에게도 관리자
    토큰을 줘야 했고, 그 토큰의 쿠키(path=/)가 관리자 콘솔 로그인까지 겸했다 — 설계단계 감리의
    "골든셋 검증 페이지에서 관리자 콘솔로 접근 가능" 지적의 뿌리다. 검수자에게는 관리자 전용 조작을 감춘다.
    """
    _console_actor_id(auth)
