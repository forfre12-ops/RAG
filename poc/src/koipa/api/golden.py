...
from koipa.console_nav import HEADER_CSS, NAV_CSS, header_html, upload_progress_js
...
function actionLabel(){const sel=$('action'),opt=sel&&sel.selectedOptions&&sel.selectedOptions[0];
return (opt?opt.textContent:'').replace(/\s*\(.*\)$/,'').trim()||'이 결정'}
// [2026-08-25] 비밀관리성(M)은 등급 결정의 **입력**이다(decide docstring). 등급을 정하지
// 않는 결정(폐기·검수 대상 아님·재검토)에서는 블록째 감춘다 — 잠긴 칸을 남겨 두면 왜
// 잠겼는지 설명할 줄이 또 필요해진다. 감춘 값은 save() 가 보내지 않는다(disabled).
// 「출처 기록」은 결정이 아니라 문서의 속성이라 여기서 건드리지 않는다.
function syncMgmt(){const w=$('mgmtWrap');if(!w)return;
const used=['change','approve','defer'].includes($('action').value);
w.style.display=used?'':'none';$('secMarking').disabled=!used;$('accScope').disabled=!used;}
function history(events){let box=$('history');box.replaceChildren();if(!events.length){box.textContent='결정 이력이 없습니다.';return}events.slice().reverse().forEach(e=>{let d=document.createElement('div');d.className='event';let a=document.createElement('b');a.textContent=(e.action||'결정')+' · '+(e.status||'');let w=document.createElement('div');w.className='when';w.textContent=(e.decided_at||'')+' · '+(e.actor_id||'');let r=document.createElement('div');r.textContent=e.reason||'사유 없음';d.append(a,w,r);box.append(d)})}async function load(){try{let session=await req(api+'/session');$('session').textContent='로그인 관리자 · '+session.actor_id;let data=await req(qurl());renderRows(data);msg(data.total+'건을 불러왔습니다.'+(data.listed_excludes_discarded&&data.summary.discarded?' (폐기 '+data.summary.discarded+'건은 목록에서 뺐습니다 — 상태 필터를 폐기로 두면 보입니다)':''))}catch(e){msg('불러오기 실패: '+e.message,true)}}function provLabel(c){if(!c.is_actual_document)return '해당 없음(합성)';
...
$('provReason').value='';
$('provOrigin').textContent=p.origin?('기록 방식: '+(({legacy_top_level:'초기 일괄 등록분에서 승계',manual:'화면에서 직접 입력',upload:'파일 업로드 때 함께 기록'})[p.origin]||p.origin)):'';
$('provSave').disabled=(p.status==='recorded');note('provMsg',p.status==='recorded'?'기록 완료 — 더 채울 것이 없습니다.':'');}
// [2026-08-23] 비밀관리성(M) 표시. 판정은 서버(rule_engine)가 하고 화면은 그 결과만
// 보여준다 - 여기서 다시 계산하면 판정식이 두 곳에 생긴다.
function renderMgmt(c){const w=$('mgmtWrap');if(!w)return;const m=c.management||{};
$('secMarking').value=m.security_marking||'';$('accScope').value=m.access_scope||'';
const box=$('mgmtState');const st=m.state||'unknown';
if(st==='present'){box.style.color='#3f6212';
 box.innerHTML='현재 <b>M='+m.level+'</b> — 관리 수준이 확인됐습니다.';}
else if(st==='proven_absent'){box.style.color='#9a3412';
 box.innerHTML='현재 <b>M=0</b> — 전 임직원 열람이므로 비밀관리성 요건 미충족이 <b>입증</b>됐습니다. 곱이 0이 되어 등급이 내려갑니다.';}
else{box.style.color='#8d949b';
 box.innerHTML='현재 <b>M 확인 안 됨</b> — 보수적으로 2로 채웁니다. 이 상태에서는 <b>S1이 나올 수 없습니다</b>(S1은 M=0으로 확정될 때만 나오는 조합입니다).';}}
...
msg('출처를 기록했습니다. 등급·상태는 바뀌지 않습니다.');await show(selected.doc_id);note('provMsg','출처를 기록했습니다. 등급·상태는 바뀌지 않습니다.')}
catch(e){msg('출처 기록 실패: '+e.message,true);note('provMsg','출처 기록 실패 — '+e.message,true)}}
async function show(id){try{let c=await req(api+'/'+encodeURIComponent(id));selected=c;$('detailTitle').textContent=c.doc_id+' · '+c.title;$('metas').replaceChildren();[['상태',c.status],['출처',c.document_origin],['제안',c.proposed_grade||'없음'],['확정',c.final_grade||'미확정'],['글자 수',c.characters.toLocaleString()],['SHA-256',c.document_sha256.slice(0,14)+'…'],['출처 기록',provLabel(c)]].forEach(x=>{let m=document.createElement('span');m.textContent=x[0]+': '+x[1];$('metas').append(m)});$('scope').textContent=c.claim_scope;$('document').textContent=c.text;$('documentRendered').innerHTML=mdToHtml(c.text);$('finalGrade').value=c.final_grade||c.proposed_grade||'S3';$('reason').value='';note('saveMsg','');note('provMsg','');setActions(c);renderProv(c);renderMgmt(c);history(c.decision_history||[]);$('detail').classList.add('show');$('detail').scrollIntoView({behavior:'smooth',block:'start'});await load()}catch(e){msg('상세 조회 실패: '+e.message,true)}}async function save(){try{if(!selected)throw new Error('문서를 먼저 선택하세요.');const action=$('action').value,reason=$('reason').value.trim();if(['change','defer','discard','exclude'].includes(action)&&!reason)throw new Error('「'+actionLabel()+'」에는 사유가 필요합니다.');let body={action,reason};if(action==='change')body.grade=$('finalGrade').value;const sm=$('secMarking'),asc=$('accScope');if(!sm.disabled&&sm.value)body.security_marking=sm.value;if(!asc.disabled&&asc.value)body.access_scope=asc.value;let out=await req(api+'/'+encodeURIComponent(selected.doc_id)+'/decision',{method:'POST',body:JSON.stringify(body)});msg('저장 완료: '+out.status);await show(selected.doc_id);note('saveMsg','저장했습니다 · 상태 '+out.status)}catch(e){let t=e.message||'';if(t.indexOf('missing_provenance')>=0){t='실문서는 원천 위치와 사용 권한 근거를 모두 기록해야 등급을 확정할 수 있습니다 — 아래 「출처 기록」에서 채운 뒤 다시 저장하십시오. (보류·폐기는 지금도 됩니다)';const f=$('provSrc');if(f&&f.focus)f.focus();}msg('저장 실패: '+t,true);note('saveMsg','저장 실패 — '+t,true)}}""" + DOC_RENDER_JS + r"""function setDocView(rendered){
...
async function upload(){try{let file=$('file').files[0];if(!file)throw new Error('업로드할 파일을 선택하세요.');let src=$('upSource').value.trim();let selv=($('upBasisSel')||{}).value||'';let bas=(selv&&selv!=='__custom__')?selv:$('upBasis').value.trim();/* [2026-08-23] 두 칸의 필수 강제를 뺐다 — 게이트는 등급 확정(decide)과 승격(promote_to_locked)으로 옮겼다. 현관에서 막으니 평가셋이 보호된 게 아니라 등록 자체가 안 됐다(실측 2026-08-17 223: 실문서 74건 중 62건 미완). */let form=new FormData();form.append('file',file);form.append('document_origin',$('upOrigin').value);form.append('source_reference',src);form.append('authorization_basis',bas);let ub=$('upload');if(ub){ub.disabled=true;ub.dataset.label=ub.textContent;ub.textContent='분석 중…';}window.UploadProgress.start(file,'문서를 등록하고 본문을 뽑고 있습니다');msg('문서를 분석하고 있습니다 — '+file.name+' 에서 글자를 뽑는 중입니다. 파일이 크면 몇 초 걸립니다.');let out=await req(api+'/upload',{method:'POST',body:form,json:false});$('modal').classList.remove('show');$('upSource').value='';$('upBasis').value='';if($('upBasisSel')){$('upBasisSel').value='';$('upBasis').style.display='none';}$('file').value='';if($('dropName'))$('dropName').textContent='여기로 파일을 끌어다 놓거나 위에서 선택하세요';msg('업로드 완료: '+out.doc_id+' · 검토 대기 상태입니다.');await load();await show(out.doc_id)}catch(e){msg('업로드 실패: '+e.message,true)}finally{window.UploadProgress.stop();let b=$('upload');if(b){b.disabled=false;b.textContent=b.dataset.label||'업로드 및 추출';}}}
...
def _render_console_login_html() -> str:
    ...
    return (
        ...
        "var v=$('t').value.trim();if(v)login(v,true)})();"
        "</script></body></html>"
    )
