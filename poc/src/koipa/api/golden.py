...
# 살아 있는 렌더러는 _render_specledger_gold_console_html / _render_actual_document_intake_html
# / _render_console_login_html 셋이다.
...
$('refresh').onclick=load;$('filter').onclick=load;$('action').onchange=toggleGrade;$('provSave').onclick=saveProv;$('save').onclick=save;$('openUpload').onclick=()=>$('modal').classList.add('show');$('cancelUpload').onclick=()=>$('modal').classList.remove('show');$('upload').onclick=upload;$('ledgerReload').onclick=loadLedger;$('ledgerFilter').onchange=loadLedger;
...
def _render_console_login_html() -> str:
    ...
    return (
        ...
        '<div class="product">AI 영업비밀 · 골든셋 검수 로그인</div>'
        # 로그인 화면은 아직 쿠키가 없다 - 링크를 눌러도 401 이 정상이다.
