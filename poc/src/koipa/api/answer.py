...
from koipa.schemas.rag_answer import RagAnswerRequest, RagAnswerResult
...
def _fetch_hits(req: RagAnswerRequest) -> list[RagContextHit]:
    ...
    tenant 제거: 격리는 KL 포털 전담(상류 보장). Koipa는 단일 고객사 엔진이라
    내부에 per-customer 경계가 없어 검색을 무스코프(filter_=None)로 수행한다.
    """
    ...
    # tenant 제거: 격리는 KL 포털 전담 → 검색 무스코프.
    filter_ = None
