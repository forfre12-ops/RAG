...
try:
    from koipa.api.prom_metrics import registry as _prom_registry
    from prometheus_client import Counter as _Counter

    AUDIT_WRITE_FAILURE_TOTAL = _Counter(
        "koipa_audit_write_failure_total",
...
def _fail_closed_enabled() -> bool:
    ...
    KOIPA_AUDIT_FAIL_CLOSED_HIGH_RISK 폴백(컨테이너 재빌드 없이 운영 토글).
    """
    ...
    try:
        ...
        pass
    return os.getenv("KOIPA_AUDIT_FAIL_CLOSED_HIGH_RISK", "").strip().lower() in {
        "1", "true", "yes", "on",
    }
...
def _try_build_chained_hash(body: bytes) -> str | None:
    ...
    try:
        from koipa.services.audit_chain import build_chained_hash, get_last_hash  # noqa: PLC0415
    except ImportError:
        return _hash_bytes(body) if body else None
    try:
        prev = get_last_hash()
...
class AuditMiddleware(BaseHTTPMiddleware):
    ...
    def _record(
        *,
        ...
        try:
            from koipa.services.audit_chain import build_chained_hash_locked  # noqa: PLC0415
        except ImportError:
