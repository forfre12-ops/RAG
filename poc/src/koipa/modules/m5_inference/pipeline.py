...
# hardened42 에서 배포본이 정답 S1 을 S2 로 자동확정한 건이 있었다(confidence 0.843 ·
# 룰도 S2 에 합의 · 경보 0건). 요소값이 규칙 분기점에 얹혀 있어서였다. 그래서 "룰이 뽑은
# S/V/M 을 한 단계 올려 보고 등급이 높아지면 검수로 보낸다"는 게이트를 붙여 봤다.
#
# 실측 결과 두 변형 모두 채택 불가였다:
#   1단계 탐침  전 조합의 59%(16/27) 가 경계 · hardened42 자동확정 40.5%->33.3%
#               잡힌 3건은 **전부 예측이 맞은 건** — 미탐은 하나도 못 고쳤다
#   2단계 탐침  전 조합의 93%(25/27) 가 경계 · 자동확정 40.5%->16.7%
#               미탐 1건은 잡지만 사실상 자동화 해제다
# 1단계가 못 잡은 이유: 룰이 S1·V1·M1 로 봤고 심판은 s=2·v=1.67 이라 **두 축이 동시에**
# 낮았다. 한 축만 올려서는 닿지 않는다.
#
# 즉 진짜 결함은 분기점이 아니라 **룰 추출기의 체계적 과소검출**이다(hardened42 N=24 에서
# 룰-심판 차이 S 평균 -0.79·58% 가 낮음, V 평균 -0.72·50% 가 낮음, M 은 +0.33 로 정상).
# 경계 게이트는 그 증상 위에 검수부담만 얹는다. 요소 추출을 고치는 것이 답이고,
# 그건 별건이다. 되돌린 이유를 남긴다 — 같은 아이디어를 다시 만들지 않도록.
# 근거: docs/V7_RESULT_2026-08-12.md §4
