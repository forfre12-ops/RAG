...
def apply_serving_gate(codes, probs, *, metadata=None, tau: float = 0.99,
    ...
    #   미탐이 아니라는 것은 맞지만 **맞다는 뜻은 아니다.** 자동확정분은 검수 큐를 건너뛰어
    #   확정 대기(staging, FUN-005 "임시저장 → 관리자 최종확정")로 가므로, 관리자가 제시
    #   등급을 그대로 승인하면 그 문으로 과분류가 확정된다.
    #   실측(tmp/sweep_clean_v8_caus.jsonl 1,055건): 자동확정 자격 509건이 전부 TS 였고
    #   그 라벨은 TS 174 · S1 115 · S2 73 · S3 147 — **65.8%가 TS 가 아니었다.**
    #   3요소를 다 확정한 행은 16/1,055 = 1.5% 다. 이를 만드는 것은 factor_kappa(0.99)이고
    #   (reports/FACTOR_THRESHOLD_SWEEP_2026-09-10.json), 문턱을 내리면 자동확정분의
    #   70.8~91.7% 가 낮게 본 쪽으로 틀린다. 그 공백을 자동확정으로 덮지 않는다.
