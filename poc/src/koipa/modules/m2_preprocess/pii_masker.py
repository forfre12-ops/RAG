...
_PII_PATTERNS: list[tuple[str, re.Pattern[str], str]] = [
    ...
    # [2026-08-24] 구분자를 **필수**로 바꿨다(`[- ]?` → `[- ]`). 구분자 없는 13자리는
    # 아래 _RRN_NOSEP_RE + 체크섬으로 따로 본다 — 사업자등록번호에 이미 쓰는 방식이다.
    #
    # 왜(실측 2026-08-24): 종전 패턴이 구분자 없는 13자리를 다 잡아 **등록번호를 개인정보로
    # 오인**했다. 「등록번호 제4100302980000호」(상표 등록번호)가 [RRN] 으로 가려졌다.
    # 앞뒤 lookaround 는 괄호·한글이 오면 막지 못한다. 판례·공고문에 흔한 값이라 그대로 두면
    # 본문이 훼손되고 분류 신호까지 지워진다.
    # [2026-08-24 재수정] **구분자 있는 형태는 무조건**, 구분자 없는 13자리는 한글에 붙어
    # 있으면 제외한다.
    #
    # 경과: 처음엔 구분자를 필수로 바꿨는데(오탐 제거 목적) 그것이 무하이픈 우회 차단을
    # 없앴다 — tests/test_audit_c_ingest.py::TestPiiNoHyphenBypass 가 그 자리에서 잡았다.
    # 체크섬으로 가르려 했으나 그 시험이 쓰는 8001011234567 은 체크섬이 맞지 않는 가상
    # 번호라 쓸 수 없다(계산 결과 검증숫자 0 vs 실제 7).
    #
    # 실제 오탐은 전부 **한글에 붙은 등록번호**였다(실측):
    #   「등록번호 제4100302980000호」  앞 제 · 뒤 호
    # 반면 진짜 PII 는 공백·구분자로 떨어져 있다(「주민 8001011234567 등록」).
    # 그래서 구분자 없는 형태에만 한글 인접 금지를 건다. 우회 차단은 그대로 유지된다.
    #
    # ⚠ 남는 오탐: 「상표등록 4100302980000」처럼 공백으로 떨어진 등록번호는 여전히 가려진다.
    #   안전(PII 보호)과 정밀(본문 보존)이 부딪히면 **안전을 택한다.** 되레 놓치는 쪽이 위험하다.
    ("rrn", re.compile(r"(?<!\d)\d{6}[- ][1-4]\d{6}(?!\d)"), "[RRN]"),
    # ⚠ 이름을 "rrn" 으로 같게 둔다 — counts 는 이름으로 합산되고, 그 키는 감사 기록의
    #   계약이다(test_audit_c_ingest 가 counts["rrn"] 을 본다). 구분자 유무는 구현 세부다.
    ("rrn", re.compile(r"(?<![0-9가-힣])\d{6}[1-4]\d{6}(?![0-9가-힣])"), "[RRN]"),
    ("frn", re.compile(r"(?<!\d)\d{6}[- ][5-8]\d{6}(?!\d)"), "[FRN]"),
    ("frn", re.compile(r"(?<![0-9가-힣])\d{6}[5-8]\d{6}(?![0-9가-힣])"), "[FRN]"),
    ...
    # [2026-08-24] 구분자 없는 형태를 **010 + 8자리**로 좁혔다. 종전 `\d{3,4}` 는 10자리도
    # 잡아 「공매개시번호(0160010014)」를 [PHONE] 으로 가렸다(실측). 구분자가 있으면
    # 옛 10자리 번호(016-001-0014)도 그대로 잡힌다.
    ("phone_mobile", re.compile(r"\b01[016789][- ]\d{3,4}[- ]\d{4}\b"), "[PHONE]"),
    ("phone_mobile_nosep", re.compile(r"(?<!\d)010\d{8}(?!\d)"), "[PHONE]"),
...
def mask_pii(
    ...
    *,
    ...
    if counts:
        try:
            from koipa.api.prom_metrics import PII_MASKED_TOTAL  # noqa: PLC0415
