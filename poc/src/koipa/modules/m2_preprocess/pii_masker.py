...
_PII_PATTERNS: list[tuple[str, re.Pattern[str], str]] = [
    ...
    # 주민/외국인등록번호 — 구분자가 있으면 무조건 마스킹, 구분자 없는 13자리는 한글에 붙어
    # 있으면 제외한다(무하이픈 우회 8001011234567 은 계속 잡는다). 7번째 자리 [1-4]/[5-8]
    # 제약으로 임의 13자리 오탐을 줄인다.
    # 한글 인접 제외의 근거(실측 2026-08-24): 「등록번호 제4100302980000호」 같은 상표 등록번호가
    # [RRN] 으로 가려져 본문과 분류 신호가 지워졌다. 진짜 PII 는 공백·구분자로 떨어져 있다.
    # ⚠ 구분자를 필수로 바꿔 오탐을 없애는 길은 막혀 있다 — 무하이픈 우회 가드가 깨지고
    #   tests/test_audit_c_ingest.py::TestPiiNoHyphenBypass 가 그 자리에서 잡는다.
    #   체크섬으로 가르는 것도 안 된다: 그 시험의 8001011234567 은 체크섬이 맞지 않는
    #   가상 번호다(검증숫자 0 vs 실제 7).
    # ⚠ 「상표등록 4100302980000」처럼 공백으로 떨어진 등록번호는 여전히 가려진다 —
    #   안전(PII 보호)과 본문 보존이 부딪히면 안전을 택한다.
