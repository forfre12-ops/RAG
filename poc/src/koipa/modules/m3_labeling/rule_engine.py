...
# 관리성은 본문에서 관측되지 않는다 — 실문서 판정면은 중앙값 167~236자이고 관리성 어휘를
# 가진 것이 TS 9/33 · S1 2/22 · S2 1/17 뿐이다(실측 2026-08-14). 증거는 보관 위치와 접근권한이다.
# 영향: 정본에서 S1 이 나오는 조합은 (2,2,0) 하나뿐이라 M 을 못 받으면 S1 은 구조적으로
# 도달 불가이고, 판정면의 28%(18/64)가 어떤 모델에서도 TS 가 된다(4등급 정확도 상한 0.719).
# 매핑은 새로 만드는 것이 아니라 KL 과 합의된 ICD §3.2·§3.3 의 구현이다.
# ⚠ all_employees → M=0 은 하향 경로다(s=2 v=1 m=0 → S3). 법리상 맞지만 미탐 방향이므로
#   호출부가 검수로 보낼 수 있게 근거(reason)를 함께 돌려준다.
...
_MANAGEMENT_MARKING_TERMS: tuple[str, ...] = (
    ...
    # [2026-08-22] 조사가 끼면 못 잡던 문장형 표기. 실측: 시연 문서
    # 07_HYBRID_semantic_secret.docx 는 "관계자 외 열람을 제한합니다" · "지정된 인원 외에는
    # 공유하지 않는다" 를 본문에 달고도 이 목록에 하나도 안 걸렸다 - "열람 제한" 은 있는데
    # "열람을 제한" 이 없어서다. 목록이 이미 잡으려던 표현이므로 변형을 채운다(탐지 전용,
    # 등급 무영향은 그대로).
    "열람을 제한", "열람이 제한", "열람을 금지", "열람 금지",
    "접근을 제한", "접근이 제한",
    "관계자 외", "관계자외", "지정된 인원", "지정 인원", "허가된 인원", "승인된 인원",
...
class LabelRuleEngine:
    ...
    def _get_embedder(self):
        ...
        우선순위:
          ...
          3. 그 외에는 koipa.adapters.embedding.build_embedder()로 폴백
        """
...
def build_rule_engine_from_db(**kwargs) -> LabelRuleEngine:
    ...
    """
    from koipa.modules.m3_labeling.seeds import KEYWORD_SEEDS, load_seeds_from_db  # noqa: PLC0415
