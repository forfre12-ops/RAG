...
# [2026-08-24] 주민등록번호 패턴을 마스커와 같은 정밀도로 맞췄다.
#
# 종전 `\d{6}-\d{7}` 은 **법인등록번호와 형식이 같아서** 그것까지 PII 위반으로 셌다.
# 실측(배포 모델 학습셋 labeled_p1_retrain_v4_clean, 합성 3,187행): 위반 2건이 잡혔는데
# 열어 보니 둘 다 법인등록번호였다 —
#   「설립등기(등록번호 : 200131-0000275)」
#   「주식회사 폭스코리아(등록번호: 000000-0000000)」  ← 판례가 이미 익명화한 값
# 즉 **실제 PII 위반은 0건**이고, 이 계측기가 0.1% 를 잘못 세고 있었다.
#
# 마스커(m2_preprocess/pii_masker.py:28)는 이미 제대로 가른다 — 주민번호는 7번째 자리를
# [1-4] 로 제약하고, 법인등록번호는 corp_reg 로 따로 둔다. 같은 기준을 여기에도 쓴다.
# 앞뒤 숫자 lookaround 도 함께 가져온다(계좌·일련번호의 부분 매치 방지).
_PII_PATTERNS = [
