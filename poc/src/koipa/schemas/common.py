...
def _registry_ttl_sec() -> float:
    """레지스트리 캐시 TTL(초). KOIPA_REGISTRY_CACHE_TTL_SEC 로 조정.
    ...
    """
    raw = os.getenv("KOIPA_REGISTRY_CACHE_TTL_SEC", "30").strip()
    try:
        return float(raw)
    except ValueError:
        return 30.0
...
class FactorRegistry:
    ...
    def get_field_map(cls) -> dict[str, str]:
        ...
        try:
            from koipa.db import session_scope  # noqa: PLC0415
            from koipa.db.models import EvaluationFactor  # noqa: PLC0415
            with session_scope() as db:
                factors = (
