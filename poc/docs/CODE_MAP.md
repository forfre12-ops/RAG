...
소스는 **182 파일 · 33,803줄**이지만, 요건 하나를 확인하는 데 필요한 파일은 보통 **4~5개**입니다.
...
| 층 | 파일 수 | 하는 일 | **하지 않는 일** |
|---|---|---|---|
| `api/` | 25 | 요청·응답 계약, 인증·권한, 상태코드 | 비즈니스 판단, DB 직접 접근 |
| `services/` | 26 | 유스케이스 조율, 트랜잭션 경계 | 알고리즘 구현, HTTP 처리 |
| `modules/` | 48 | AI 파이프라인 알고리즘 (m1~m6) | HTTP, 인증, 요청 컨텍스트 |
| `adapters/` | 28 | 임베딩·벡터스토어·LLM·스토리지 경계 | 도메인 판단 |
| `repositories/` | 9 | DB 읽기·쓰기 | 비즈니스 규칙 |
| `schemas/` | 14 | Pydantic 계약 | 로직 |
| `workers/` | 3 | Celery 비동기 (학습 등 장시간 작업) | 동기 응답 |
