...
(2026-08-20 문구 개정. 종전에는 「~이다」·「~할 것」 이 「~습니다」와 섞여 있었고,
검수자에게 설정 이름부터 들이밀었다. 지금은 **할 일을 먼저** 말하고 설정 이름은
「운영자:」 괄호로 뒤에 둔다.)

```
로그인이 되어 있지 않습니다. 콘솔에 로그인한 뒤 다시 여십시오.
자리표시 이름입니다('reviewer'). 본인 계정으로 로그인하십시오.
기계·시연 전용으로 예약된 이름입니다('demo-console' — 'demo' 로 시작). 사람 계정으로 로그인하십시오.
서명 화면에 기본으로 채워지는 이름입니다('hong.gildong'). 누가 검수했든 같은 이름이 남기
때문에 개별 검수로 인정하지 않습니다. 관리자에게 본인 계정 토큰을 요청하십시오.
(운영자: 설정 SIGNOFF_DEFAULT_REVIEWER 에서 이 이름을 빼십시오.)
로그인 화면이 모두에게 나눠 주는 공용 이름입니다('kl-admin-test'). 이 화면을 여는 누구나
같은 이름을 받기 때문에 누가 검수했는지 남지 않습니다. 관리자에게 본인 계정 토큰을
요청하십시오. (운영자: 검수자 토큰은 설정 CONSOLE_LOGIN_PREFILL_TOKEN 프리필에 넣지 말고
사람마다 따로 발급하십시오.)
```
