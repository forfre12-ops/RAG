...
TRAILING = {
    ...
    # [2026-08-24] 구역 이동 링크 4개(시연·운영·반영·법령·문서 업로드)를 **뺐다**(사용자 지시,
    # 두 번째 요청). 같은 줄에 메뉴가 두 벌 있었다 — 왼쪽 cnav 4개는 **화면 사이** 이동이고
    # 이 4개는 **같은 화면 안** 구역 이동이라, 생김새가 같은데 하는 일이 달라 어느 것이 화면
    # 이동인지 구별되지 않았다. 화면 안 이동은 §번호 목차와 스크롤로 이미 된다.
    # #sec-parse 로 들어오는 외부 링크는 admin.html cnav("등급 시연")가 유지한다.
    "index.html": "",
}
