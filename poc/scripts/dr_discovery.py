"""DR 백업/복구 공용 — 실행 중 lloydk compose 컨테이너 자동탐지.

scripts/ 하위 백업·복구 스크립트가 공유한다(각 스크립트는 `python scripts/X.py` 로 실행돼
sys.path[0]=scripts 라 상대 import 가능). 컨테이너명이 배포마다 다르다:
  dev=lloydk-poc-* · airgap=lloydk-airgap-* · dual=lloydk-jjw-*·lloydk-cust-*
그래서 하드코딩 기본값(lloydk-poc-postgres-1)이 airgap·dual 에서 빗나가 백업/복구가 조용히
...
def list_lloydk_containers() -> list[dict[str, str]]:
    """실행 중 lloydk compose 컨테이너 — [{name, project, service}, ...].
    ...
    for line in r.stdout.splitlines():
        ...
        if not name or not project.startswith("lloydk"):
            continue
...
def autodetect_container(
    ...
    """
    containers = list_lloydk_containers() if containers is None else containers
