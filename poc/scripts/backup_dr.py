"""DR 백업 오케스트레이터 — 실행 중 모든 koipa 스택의 PG + 원문 스토리지를 한 번에 백업.

배경:
- 이전엔 backup_postgres.py 만 있었고 (a) PG만 덤프(원문 스토리지 백업 없음), (b) 컨테이너
  기본값이 dev 명(koipa-poc-*)이라 airgap/dual 에서 빗나가고, (c) cron 에 배선돼 있지 않았다.
...
스케줄링(호스트 cron — worker 컨테이너엔 docker 소켓이 없어 beat 로는 실행 불가):
  ...
  0 2 * * * cd /opt/koipa/poc && /usr/bin/python scripts/backup_dr.py >> /var/log/koipa-dr-backup.log 2>&1
또는 systemd 타이머: infra/systemd/koipa-dr-backup.{service,timer} 참조.
...
def main(argv: list[str] | None = None) -> int:
    ...
    p = argparse.ArgumentParser(description="Koipa DR 백업 오케스트레이터(pg+storage, fail-closed)")
    ...
    stacks = group_stacks()
    if not stacks:
        logger.error("실행 중 koipa 스택 없음(postgres + worker/api 동시 필요) — 백업 대상 없음")
        return 2
