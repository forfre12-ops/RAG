...
각 단계마다 대응하는 콘솔 카드를 함께 찍어, 터미널 시연과 화면 시연이 같은 대본이 되게 한다.
  ...
  ① 업로드      (추출·정규화·청킹·마스킹·원문저장)   ← 분류 콘솔의 파싱·분류 시연 구역
...
되돌리기 주의 (2026-08-08 실측):
  이 스크립트가 만든 데이터는 화면 경로(/demo/index.html#sec-parse)와 같은 마커를 단다 —
...
def main() -> int:
    ...
    with httpx.Client(timeout=180.0) as cli:
        ...
        step("①", "업로드 → 텍스트추출·정규화·청킹·마스킹·원문저장", "분류 콘솔 · 파싱·분류 시연 구역 (문서 업로드)")
