...
def _to_gold_record(r: dict, base: dict | None = None, *, as_candidate: bool = False) -> dict:
    """Build a gold record, preserving fields of an existing record when
    promoting a pseudo-labeled doc.

    as_candidate=True([G2] 고객사 반입 전용): human_review(=locked_gold_eval)로 승격하지 않고
    label_source=customer_review + review_status=gold_candidate로 기록 → golden_tiers.tier_of가
    TIER_CANDIDATE(학습용, 평가 정답 아님)로 분류한다. 고객 라벨이 eval 홀드아웃을 오염
    (train-on-test)하는 것을 원천 차단. (기본 as_candidate=False = 지재원 골든셋 검수 승격, 기존 동작 보존.)
    """
    ...
    rec.update({
        ...
        "label_source":   "customer_review" if as_candidate else "human_review",
        "reviewer_id":    r["reviewer_id"],
        "review_status":  "gold_candidate" if as_candidate else "accepted",
...
def merge_into_gold(records: list[dict], dry_run: bool, *, as_candidate: bool = False) -> int:
    """text가 있는 human_label 검수 건을 gold_real에 편입.

    as_candidate=True([G2] 고객사 반입)이면 human_review(locked_gold_eval)가 아니라
    gold_candidate(학습용)로 편입 — 고객 라벨이 평가 정답을 오염하지 않게 한다.
    ...
    tier = "gold_candidate(학습용)" if as_candidate else "human_review(평가정답)"
    promoted = added = 0
    for r in mergeable:
        if r["doc_id"] in index:
            i = index[r["doc_id"]]
            existing[i] = _to_gold_record(r, base=existing[i], as_candidate=as_candidate)
            promoted += 1
        else:
            existing.append(_to_gold_record(r, as_candidate=as_candidate))
            added += 1

    if dry_run:
        print(f"[DRY-RUN] gold_real {tier} 승격 {promoted}건, 신규 {added}건")
        return promoted + added

    _save_jsonl(GOLD_REAL_PATH, existing)
    print(f"[OK] gold_real {tier}: 승격 {promoted}건, 신규 {added}건 (총 {len(existing)}건)")
    return promoted + added
...
def write_to_db(records: list[dict], dry_run: bool, *, as_candidate: bool = False) -> dict:
    """human_review 검수 결과를 document_labels 테이블에 upsert.

    as_candidate=True([G2] 고객사 반입)이면 labeled_by=customer_review·is_verified=False로
    기록 — 서빙 exact-match override·유사도 게이트 refset이 고객 라벨을 authoritative(검증됨)로
    취급하지 않게 한다(승급은 사람 승인 PromotionService 경유).
        ...
        for r in records:
            if r.get("text"):
                print(f"  [DRY-DB] DocumentLabel upsert: doc_id={r['doc_id']} "
                      f"label={r['human_label']} reviewer={r['reviewer_id']} as={labeled_by}")
    ...
    try:
        ...
        for r in records:
            ...
            # 2. DocumentLabel upsert ([G2] as_candidate → customer_review·is_verified=False)
            _vby = r.get("reviewer_id", "human") if verified else None
            _notes = (
                f"reason_code={r.get('reason_code','')} "
                f"model={r.get('model_label','')}→human={r['human_label']}"
                + ("" if verified else " [candidate: 고객사 반입·미검증]")
            )
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--write-db",   action="store_true",
                   help="document_labels 테이블에 label upsert (DB 연결 필요)")
    p.add_argument("--as-candidate", action="store_true",
                   help="[G2] 고객사 검수 반입 전용 — human_review(locked_gold_eval) 대신 "
                        "gold_candidate(학습용)로 기록. 고객 라벨이 eval 정답을 오염(train-on-test)하는 것을 "
                        "차단한다. 기본(미지정)=지재원 골든셋 검수 승격(human_review).")
...
def main() -> int:
    ...
    if args.as_candidate:
        print("[G2] as_candidate 모드 — 편입/기록을 gold_candidate·customer_review(학습용, 평가정답 아님)로 처리")

    if args.dry_run:
        print("[DRY-RUN] 실제 파일 수정 없음")
        if args.merge_gold:
            merge_into_gold(valid, dry_run=True, as_candidate=args.as_candidate)
        if args.write_db:
            write_to_db(valid, dry_run=True, as_candidate=args.as_candidate)
        return 0
    ...
    if args.merge_gold:
        merge_into_gold(valid, dry_run=False, as_candidate=args.as_candidate)

    # ── DB write (--write-db) ────────────────────────────────────────────────
    if args.write_db:
        db_result = write_to_db(valid, dry_run=False, as_candidate=args.as_candidate)
