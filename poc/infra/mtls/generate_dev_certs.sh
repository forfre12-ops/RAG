#!/usr/bin/env bash
# P2-C7: 개발용 mTLS 인증서 자체 발급 스크립트.
# 운영 환경에서는 사내 PKI 또는 cert-manager(Let's Encrypt + intermediate CA) 사용.
...
openssl req -x509 -new -nodes -key ca.key -sha256 -days 1825 \
    -subj "/CN=Lloydk Dev CA" -out ca.crt

# 2) 서버
openssl genrsa -out server.key 4096
openssl req -new -key server.key -out server.csr \
    -subj "/CN=api.lloydk.local"
cat > server.ext <<EOF
