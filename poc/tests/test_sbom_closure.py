...
섞였다. closure scope는 koipa-ai 선언 의존 + 선택 extras의 transitive 폐포만 산출한다.
...
_FAKE_REQUIRES = {
    "koipa-ai": ["fastapi>=1", "torch>=2 ; extra == 'full'", "pytest ; extra == 'dev'"],
...
def test_closure_includes_base_and_selected_extra(fake_requires):
    c = _dl._dependency_closure("koipa-ai", extras=("full",))
...
def test_closure_excludes_transitive_optional_extra(fake_requires):
    ...
    c = _dl._dependency_closure("koipa-ai", extras=("full", "test"))
    assert "playwright" not in c


def test_closure_excludes_self(fake_requires):
    c = _dl._dependency_closure("koipa-ai", extras=())
    assert "koipa-ai" not in c
