...
[2026-09] 라벨에서 '+rag' 를 뗐다 — 유사문서 검색을 폐기했고, 애초에 그 단계가 등급에
관여한 적도 없어 라벨이 하지 않은 일을 말하고 있었다. 판정 대상은 그대로 rule vs model 이다.
"""
...
def test_rule_paths_labeled_rule(model_version):
    assert _method_label(model_version) == "rule"
...
def test_model_paths_labeled_model(model_version):
    assert _method_label(model_version) == "model+rule"


def test_rule_fallback_not_mislabeled_as_model():
    # 회귀 핵심: 실제 폴백 문자열이 model 로 둔갑하면 안 된다.
    assert _method_label("rule-fallback-v0") != "model+rule"
