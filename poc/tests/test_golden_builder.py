"""lloydk.golden_builder.build_golden_set 코어 파이프라인 단위 테스트.

라벨러를 주입(label_fn)해 DB/Celery/LLM 없이 위생(중복·누출)·합의·조립을 검증한다.
"""
from lloydk.golden_builder import LabelPair, build_golden_set

# 합의 게이트(G1) 기준: 동의+고신뢰 → gold_consensus / 불일치 → uncertain(disagree)
_CONSENSUS = LabelPair("S1", 0.8, "S1", 0.9)   # gold
_DISAGREE = LabelPair("S2", 0.6, "S1", 0.9)    # uncertain
...
def test_gold_and_uncertain_bucketing():
    ...
    assert r.gold[0].label == "S1" and r.gold[0].label_source == "llm_judge_consensus"
    assert [x.doc_id for x in r.uncertain] == ["d2"]
    assert r.uncertain[0].label is None and r.uncertain[0].status == "disagree"
...
def test_empty_text_skipped():
    ...
    r = build_golden_set(docs, label_fn=lambda t: LabelPair("S3", 0.0, "S3", 0.8))
    assert r.stats["input"] == 2
    assert len(r.gold) + len(r.uncertain) == 1  # 빈 본문 1건 스킵
    assert r.gold[0].doc_id == "d2" and r.gold[0].status == "gold_llm_primary"
...
def test_stats_and_to_dict():
    ...
    assert d["label_source"] == "llm_judge_consensus" and d["review_status"] == "accepted"
