...
from lloydk.services import worker_metrics_bridge as bridge
...
def test_apply_audit_integrity_sets_gauges():
    ...
    assert registry.get_sample_value("lloydk_audit_chain_broken_rows") == 3.0
    assert registry.get_sample_value("lloydk_audit_chain_nil_hash_rows") == 2.0
    assert registry.get_sample_value("lloydk_audit_chain_integrity_ok") == 0.0

    # 무결성 회복 → broken/nil 0, integrity_ok 1
    pm._apply_audit_integrity({"broken": 0, "nil_hash_rows": 0, "integrity_ok": True})
    assert registry.get_sample_value("lloydk_audit_chain_broken_rows") == 0.0
    assert registry.get_sample_value("lloydk_audit_chain_integrity_ok") == 1.0


def test_apply_outbox_stats_sets_gauges():
    pm._apply_outbox_stats({"backend": "redis", "pending": 5, "dlq": 4})
    assert registry.get_sample_value("lloydk_outbox_dlq_pending") == 4.0
    assert registry.get_sample_value("lloydk_outbox_pending") == 5.0


def test_apply_drift_report_sets_gauges():
    pm._apply_drift_report({
        "lloydk_drift_kl_divergence": 0.7,
        "lloydk_drift_cosine_mean": 0.42,
        "lloydk_drift_alert": 1.0,
        "lloydk_drift_sample_size": 200.0,
    })
    assert registry.get_sample_value("lloydk_drift_kl_divergence") == 0.7
    assert registry.get_sample_value("lloydk_drift_alert") == 1.0
    assert registry.get_sample_value("lloydk_drift_sample_size") == 200.0


def test_apply_drift_report_ignores_bad_values():
    # 잘못된 값은 조용히 스킵(다른 게이지에 영향 없음).
    pm._apply_drift_report({"lloydk_drift_kl_divergence": "n/a"})
...
def test_refresh_audit_integrity_from_signal(monkeypatch):
    ...
    assert registry.get_sample_value("lloydk_audit_chain_broken_rows") == 9.0
...
def test_refresh_drift_from_signal(monkeypatch):
    monkeypatch.setattr(
        bridge, "load_signal",
        lambda name: {"lloydk_drift_kl_divergence": 0.55}
...
def test_alerts_reference_api_visible_gauges():
    ...
    (워커 카운터 lloydk_*_total 참조로 되돌아가면 API 스크랩에서 영구 no-data 재발.)
    """
    broken = _alert_expr("AuditChainBroken")
    assert "lloydk_audit_chain_broken_rows" in broken
    assert "lloydk_audit_chain_broken_total" not in broken

    nil = _alert_expr("AuditChainNilHash")
    assert "lloydk_audit_chain_nil_hash_rows" in nil
    assert "lloydk_audit_chain_nil_hash_total" not in nil
...
def test_no_alert_references_undefined_metric_still_holds():
    ...
    src = (
        Path(__file__).resolve().parents[1] / "src/lloydk/api/prom_metrics.py"
...
def test_alert_metric_names_match_prom_definitions():
    """alert_rules.yml 의 모든 lloydk_* 참조가 prom_metrics 에 정의됐는지(전량 재검증)."""
    src = (
        Path(__file__).resolve().parents[1] / "src/lloydk/api/prom_metrics.py"
    ).read_text(encoding="utf-8")
    middleware_defined = {
