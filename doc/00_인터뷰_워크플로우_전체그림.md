...
| **Vector DB** | Qdrant | Rust 단일 컨테이너, 메타필터 강력, 멀티테넌트 분리 쉬움 |
| **RDB** | PostgreSQL 16 + pgvector | 공공기관 표준, JSONB로 가변 결과 저장 |
...
### Q3. "왜 RAG를 분류에도 쓰나요?"
> **A**: 기업마다 영업비밀 기준이 다릅니다. 같은 "매출 자료"도 A기업은 1급, B기업은 2급일 수 있습니다. 학습 모델 하나로는 이 변동성을 다 못 잡습니다. 그래서 **기업별 사내규정·정책을 Qdrant에 임베딩**(KURE-v1)해두고, 추론 시 Top-K로 가져와 모델 출력을 보강합니다. RAG는 **선택 옵션**이라 자원 부족 기업은 끌 수 있습니다.
...
### Q6. "온프레미스 배포 어떻게 하나요?"
> **A**: `docker-compose.yml` 하나로 6개 서비스(Postgres/Qdrant/MinIO/Redis/MLflow/API+Worker) 일괄 기동합니다. 모델 가중치는 이미지 외부 MinIO 볼륨이라 재빌드 없이 교체 가능. 기업 환경 도입 시 엔지니어가 현장에서 1일 설치 지원하고, 이후 운영은 기업 IT팀이 docker-compose 명령만으로 관리 가능하도록 설계했습니다.
...
| M5 추론 | 005 | 문서 텍스트 | 등급 + 신뢰도 + 근거 | KF-DeBERTa + Qdrant RAG |
...
[C] 문서 저장                              │ FK
  documents (메타+미리보기만)              │
   ...
   │  └─ tenant_id ────────────────────────┤
   ▼                                       │
  chunks (월별 파티션, vector(1024) HNSW)  │
                                           │
[D] 라벨링                                 │
...
| **"멀티테넌트 어떻게 분리해요?"** | 모든 주요 테이블에 `tenant_id`. Qdrant 네임스페이스도 테넌트 분리. 단일 테넌트(`default`) 시작 → 멀티로 확장 시 추가 마이그레이션 불필요. |
