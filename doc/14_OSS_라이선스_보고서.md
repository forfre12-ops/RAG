...
### 6.2 MinIO (AGPL-3.0) — **자체결정 v1.1 (2026-05-30): (c) 내부망 면제 해석 1순위 + (b) SeaweedFS 교체 백업 경로**

#### 6.2.0 v1.1 자체결정 잠금 (2026-05-30)

발주처 회신 안 기다리고 로이드케이 자체결정으로 다음 잠금:

- **운영 전환 시 1순위 (c)**: KOIPA 내부망 폐쇄 환경 가정 → AGPL 외부 노출 0 → 의무 발동 면제 해석 채택. 발주처 법무 답변이 면제 가능 회신이면 본 결정 그대로 진행.
- **백업 경로 (b)**: 발주처 법무 답변이 면제 불가 회신이면 §6.2.4 SeaweedFS 교체 가이드 0.5~1일 실행. compose 1줄·환경변수 1줄 변경으로 전환 가능 상태 유지.
- **제외 (a/d)**: (a) Commercial 구매는 예산 의존이라 우선순위 낮음, (d) 소스 공개는 본 사업 IP 보호 정책상 제외.

이 결정으로 발주처 회신 도착 전에도 코드·문서가 잠긴 상태로 운영 진입 가능.

#### 6.2.1 우리 시스템에서 MinIO가 하는 역할 (이하 종전 내용 동일)
