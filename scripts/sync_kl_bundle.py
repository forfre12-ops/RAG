...
(doc/result/감리정본/)에서 더 이상 끌어오지 않는다(덮어쓰기 없음). 문서는 이 폴더 안에서
...
doc/result/KL_AI자료_2026-08_미첨부문서/ 에 보관했다(정본은 doc/result/감리정본/).
...
DELIVER = {
    ...
    # 요청 4 — RFP 구현현황 근거
    "요구사항_추적표_RTM.html",
    # 요청 7(2026-08-28 접수) — 질의사항 14건 · API 통신 방안 검토
    # 회신 본문 둘은 묶음 루트에 두고, 능동 학습(가-7) 근거만 첨부로 붙인다.
    "학습_데이터_입력_경로_명세서.html",
    # [2026-08-29 정정] 위 주석은 사실이 아니었다 — 테이블정의서_ERD 는 표 수준이 아니라
    # 칼럼 상세(249개)까지 담고 있다. 확인하지 않고 별도 문서를 더 넣어 같은 내용을 두 번
    # 보내고 있었다. 한 문서로 합치고 여기서 뺀다(가-5 는 그 한 문서로 답한다).
}
